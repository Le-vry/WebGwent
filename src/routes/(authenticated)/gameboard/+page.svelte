<script>
	...
	function applyMatchState(state) {
		...
		popupVisibility =
			typeof state.popupVisibility === 'string' ? state.popupVisibility : popupVisibility;
		...
		matchCompleted =
			typeof state.matchCompleted === 'boolean' ? state.matchCompleted : matchCompleted;
	...
	function hasAvailableDecoyTargets(playerNumber) {
		...
		return ['melee', 'range', 'siege'].some((rowName) =>
			rows[rowName].units.some(isValidDecoyTarget)
		);
	}
	...
	function executeDecoySwap(targetCard, rowName, rowOwner) {
		...
		const nextHand = [...activeHand.filter((c) => c.id !== pendingDecoyCard.id), targetCard].sort(
			(a, b) => a.value - b.value
		);
	...
	function resetRoundBoardState() {
		...
		const p1WeatherCards = weather.filter((wCard) => (wCard.ownerId || 1) === 1);
		const p2WeatherCards = weather.filter((wCard) => wCard.ownerId === 2);
		if (p1WeatherCards.length) sendCardsToGraveyard(1, p1WeatherCards);
		if (p2WeatherCards.length) sendCardsToGraveyard(2, p2WeatherCards);
	...
	function placeCard(card) {
		...
		card = { ...card, ownerId: activePlayer };
	...
	function placeUnitInRow(card, ownRow, enemyRow, weather) {
		if (card.ability === 'spy') {
			...
			switch (card.ability) {
				case 'tight_bond':
					placedTightBondCard(card, ownRow.units);
					break;
				case 'horn':
					placedHornCard(ownRow);
					break;
				case 'medic':
	...
	function placedMusterCard(placedCard, row) {
		...
		const baseId = isLeader ? placedCard.M_ID.slice(0, -1) : placedCard.M_ID;
...
<main>
		...
		{#if isP1Perspective}
			...
			<div class="amountofCards" style="display:inline-flex; top: 60%;">
				...
				<img
					...
					alt="gem"
					class="gem"
					style="margin-left: 0.7vw; visibility: {p2Gem1Visibility};"
				/>
		...
		{#if !isP1Perspective}
			...
			<div class="amountofCards" style="display:inline-flex; top: 60%;">
				...
				<img
					...
					alt="gem"
					class="gem"
					style="margin-left: 0.7vw; visibility: {p1Gem1Visibility};"
				/>
	...
	<button
		class="passRound"
		disabled={!isMyTurn || isPlayerPassed(myPlayerNumber)}
		style="opacity: {isMyTurn && !isPlayerPassed(myPlayerNumber) ? 1 : 0.45};"
		on:click|preventDefault={handlePassRound}
	>
...
<aside class="Ready player">
	<div class="darken" style="display:{popupVisibility};">
		<div class="popup">
			...
			<button
				class="confirm"
				on:click|preventDefault={() => endTurn()}
				style="right: 46%; bottom: 20%;"
			>
